...
int acquire_socket_data(SockPar *sockpar, int debug, char req_mod, char req_quad);
...
#pragma omp parallel num_threads(3) private (tid) shared(sockpar)
  { tid  = omp_get_thread_num(); 
    if(tid==0){
      acquire_socket_data(&sockpar,debug, 0, 1);  // copy data from socket 
    }
    //if(tid==1){
    //  acquire_socket_data(&sockpar,debug, 1, 1);  // copy data from socket 
    //}
    else{
